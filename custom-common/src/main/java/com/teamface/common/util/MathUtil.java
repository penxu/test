...
public class MathUtil
{
    ...
    public static final Random RANDOM = new Random(System.currentTimeMillis()); // 使随机性更好
    ...
    public static boolean isNumberMath(String s)
    {
        ...
        return StringUtil.isNumber(s.substring(1));
    }
     ...
     * 
     * @param valueMin
     * @param valueMax 返回值中不包括
     * @param size 有多少个符合返回多少个
    ...
    public static int[] getRandomIntArr(int valueMin, int valueMax, int size)
    {
        ...
        {// 符合的数大于size
            int forCount = 0;
            for (int iPos = 0; iPos < size;)
            {// 防止这里出现死循环
                int ranValue = getRandomInt(valueMin, valueMax);
                if (StringUtil.isInArr(reInteger, ranValue) == -1)
                {
                ...
                forCount++;
                if (forCount > 1000000)
                {// 最多随机提取100万次就返回数组,超过就强制终止
                    String errMsg = new StringBuffer("ERROR!!! over 1000000 times at  public static int[] getRandomIntArray(").append(valueMin)
                        .append(",")
                        .append(valueMax)
                        .append(",")
                        .append(size)
     ...
     * 
     * @param valueMin 可以是负数
     * @param valueMax 可以是负数
     * @return valueMin<=return<valueMax
     */
