...
public class RegexUtil
{
    ...
    {
        if (StringUtil.isBlank(s) || s.length() < 5 || s.indexOf('@') < 1 || s.indexOf('.') < 1 || s.endsWith("@") || s.endsWith("."))
        {
            return false;
        }
     ...
     * @param @param text
     * @param @param regex
     * @param @return 参数说明 Matcher 返回类型
     */
     ...
     * @return
     */
    public static String getRegexStr(String text, String regex, Integer index)
    {
        Pattern pattern = Pattern.compile(regex);
        Matcher matcher = pattern.matcher(text);
        matcher.find();
        String regexStr = matcher.group(index);
        return regexStr;
    }
}
