...
public class StringUtil
{
    ...
    public static boolean isEmpty(final Object s)
    {
        ...
        {
           return ((JSONObject)s).isEmpty();
        }
        if (s instanceof JSONArray)
        {
           return ((JSONArray)s).isEmpty();
        }
        if (s instanceof Set)
        {
        ...
        return s.toString().trim().isEmpty();
    }
     ...
     * 功能：获取随机字符串
     * 
     * @param sRandom 随机字符串的范围
     * @param resultLength 返回字符的个数
     * @return String
     */
     ...
     * @param sOrigin
     * @param sDestination
     * @param type 1=replaceFirst 2=replaceLast 3||其它=replaceAll
     * @return
     */
    ...
    {// 把一个字符串 转换为类名的规则 首字母大写
        ...
        {// 处理所有的下划线 把紧跟着他后的字母转换大写 后面肯定是有内容的
    ...
    public static String toClassPropertyName(String s)
    {// 把一个字符串 转换为类变量名的规则
    ...
    public byte[] getBytes(char[] charArr, String charset)
    {// 将字符转为字节(编码)
    ...
    public char[] getChars(byte[] byteArr, String charset)
    {// 将字节转为字符(解码)
        if (charset == null || "".equals(charset.trim()))
        {
            charset = "UTF-8";
    ...
    {
        int result = 0;
        if (StringUtil.isBlank(versionDisplay))
        {
            return 0;
        }
        if (isMatcher(versionDisplay, "^([v|V][0-9]\\.[0-9]{1,2}\\.[0-9]{1,3})$"))
        { // 判断版本号名称是否合法
    ...
    public static String toVersionDisplay(int versionInt, String defaultName)
    {
        ...
        if (versionInt / 100000 <= 0 || versionInt / 100000 >= 10)
        { // 判断是不是六位数
     ...
     * 
     * @param dateStr 日期字符串如2015-07-25 23:59:59.000
    ...
    public static String arrayToString(String[] array, String split)
    {
        ...
        if (isEmpty(split))
        {
            split = Constant.COMMA_DELIMITER;
        }
