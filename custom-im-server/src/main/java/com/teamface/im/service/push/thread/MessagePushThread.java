 ...
 * @version: 1.0
 */
public class MessagePushThread extends MessagePushServiceAbstract implements Runnable
{
        ...
        throws Exception
    {
        ...
        if (null == result)
        {
            System.out.println(messageJson + " no push settings......");
            return JsonResUtil.getFailJsonObject();
        }
        // 检查是否有数据满足条件
        ...
        if (pushConditionOption == ImConstant.PUSH_SETTING_CONDITION_NO)
        {
            ...
            {
                pushMessage(result, json, id, companyId, pushConditionOption, tableName, pushType);
            }
        }
        // 有指定条件时推送
