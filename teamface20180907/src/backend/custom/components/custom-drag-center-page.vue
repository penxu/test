<template>
  <div class="custom-drag-center-wrip">
    <div class="custom-box-wrip">
      ...
      <div class="colum-box" v-for="(column,index) in columnNumber" :key="index">
        <div class="line">
...
export default {
  ...
  created () {
    ...
    this.$bus.on('layoutContentChange', (routerName, layoutOrPage, page) => {
      if (this.layoutIsChange(this.allLayout, this.childLayout)) {
        console.log('布局有变化')
    ...
    // 接收子布局
    this.$bus.off('setChildLayout') // 销毁
    this.$bus.on('setChildLayout', (layout) => {
      this.childLayout = layout
    })
    // 接收模块设置
  ...
  methods: {
    ...
    deleteComponent (layout, index, item) {
      ...
      let enableList = JSON.parse(JSON.stringify(this.child_unused_layout)).rows
      enableList.push(item)
      this.setUsedLayout({rows: enableList})
    },
    // 删除分栏
    ...
    getEnableLayout (data) {
      HTTPServer.getEnableLayout(data, 'Loading').then((res) => {
        ...
        this.allLayout = res
        this.childLayout = JSON.parse(JSON.stringify(res))
        this.columnNumber = res.layout
        this.headerImg = {
