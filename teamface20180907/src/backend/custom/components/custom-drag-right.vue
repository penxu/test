  ...
  <el-container class="custom-drag-right-wrip">
    <div class="module-setting" v-if="settingsType === 0">
      <el-collapse-transition>
        <div class="module-set-property" v-if="setModule">
        <p>模块设置</p>
        <el-collapse v-model="activeNames">
          <el-collapse-item name="2">
            ...
            <div class="checkbox-box">
              <el-checkbox v-model="myModule.commentControl" true-label='1' false-label='0'>评论</el-checkbox>
              <el-checkbox v-model="myModule.dynamicControl" true-label='1' false-label='0'>动态</el-checkbox>
            </div>
          </el-collapse-item>
          <el-collapse-item name="3">
            ...
            <div class="checkbox-box">
              <el-checkbox v-model="myModule.terminalPc" true-label='1' false-label='0'>PC</el-checkbox>
              <el-checkbox v-model="myModule.terminalApp" true-label='1' false-label='0'>APP</el-checkbox>
            </div>
          </el-collapse-item>
        </el-collapse>
        <a class="setting-button" @click="setModule = false"><i class="iconfont icon-xiayibu-o"></i>设置外观</a>
      </div>
      </el-collapse-transition>
      <el-collapse-transition>
        <div class="module-set-appear" v-if="!setModule">
          <el-tabs v-model="activeAppear" class="appear-tab">
            <el-tab-pane label="主题样式" name="theme">
              <div class="themes-box">
                <div class="theme" v-for="(theme, index) in themeList" :key="index" @click="shiftTheme(theme)">
                  <div class="top-image" :style="{background: theme.color}">
                    <div class="bg" :style="{'background-image': 'url(/static/img/custom/'+theme.url +')'}"></div>
                  </div>
                  <p>{{theme.text}}</p>
                </div>
              </div>
            </el-tab-pane>
            <el-tab-pane label="属性样式" name="property">
              <el-collapse v-model="activeNames" class="property-collapse">
                <el-collapse-item name="2">
                  <template slot="title">
                    <i class="el-icon-caret-right"></i>整体样式
                  </template>
                  <div class="setting-box">
                    <div class="setting-item">
                      <label for="">表单宽度</label>
                      <div class="item-content">
                        <input-number :name="'pageWidth'" :value="appearance.pageWidth" :max="900" :min="750"></input-number>
                      </div>
                    </div>
                    <div class="setting-item">
                      <label for="">页面背景</label>
                      <div class="item-content">
                        <div class="color" :style="{background:appearance.pageBgColor}" @click="openColorPicker($event,appearance.formColor,'pageBgColor','pageBgType')"><div class="arrow"></div></div>
                        <label for="pageBgImg" class="upload-file">
                          <i class="iconfont icon-icon-test"></i>
                          <input type="file" id="pageBgImg" @change="uploadFile($event, 'pageBgImg', 'pageBgType')">
                        </label>
                      </div>
                    </div>
                  </div>
                ...
                <el-collapse-item name="3">
                  <template slot="title">
                    <i class="el-icon-caret-right"></i>表头样式
                  </template>
                  <div class="setting-box">
                    <div class="setting-item">
                      <div class="item-content">
                        <el-input type="textarea" :rows="3" placeholder="请输入模块名称" v-model="headerModuleName"></el-input>
                      </div>
                    </div>
                    <div class="setting-item">
                      <label for="">头部背景</label>
                      <div class="item-content">
                        <div class="color" :style="{background:appearance.headerBgColor}" @click="openColorPicker($event,appearance.headerBgColor,'headerBgColor','headerBgType')"><div class="arrow"></div></div>
                        <el-select v-model="appearance.headerBgOpacity" placeholder="请选择" @change="transparencyChange($event, 'headerBgOpacity')">
                          <el-option
                            v-for="item in transparency"
                            :key="item"
                            :label="item"
                            :value="item">
                          </el-option>
                        </el-select>
                        <label for="headerBgImg" class="upload-file">
                          <i class="iconfont icon-icon-test"></i>
                          <input type="file" id="headerBgImg" @change="uploadFile($event, 'headerBgImg', 'headerBgType')">
                        </label>
                      </div>
                    </div>
                    <div class="setting-item">
                      <label for="">主标题</label>
                      <div class="item-content">
                        <div class="color" :style="{background:appearance.headerTextColor}" @click="openColorPicker($event,appearance.headerTextColor,'headerTextColor')"><div class="arrow"></div></div>
                        <input-number :name="'headerTextSize'" :value="appearance.headerTextSize" :max="72" :min="32"></input-number>
                      </div>
                    </div>
                  </div>
                </el-collapse-item>
                <el-collapse-item name="4">
                  <template slot="title">
                    <i class="el-icon-caret-right"></i>模块描述
                  </template>
                  <div class="setting-box">
                    <div class="setting-item">
                      <div class="item-content">
                        <el-input type="textarea" :rows="3" maxlength="25" placeholder="请输入模块描述" v-model="headerModuleDescribe"></el-input>
                      </div>
                    </div>
                    <div class="setting-item">
                      <label for="">描述文字</label>
                      <div class="item-content">
                        <div class="color" :style="{background:appearance.describeTextColor}" @click="openColorPicker($event,appearance.describeTextColor,'describeTextColor')"><div class="arrow"></div></div>
                        <input-number :name="'describeTextSize'" :value="appearance.describeTextSize" :max="32" :min="18"></input-number>
                      </div>
                    </div>
                  </div>
                </el-collapse-item>
                <el-collapse-item name="5">
                  <template slot="title">
                    <i class="el-icon-caret-right"></i>内容样式
                  </template>
                  <div class="setting-box">
                    <div class="setting-item">
                      <label for="">内容背景色</label>
                      <div class="item-content">
                        <div class="color" :style="{background:appearance.contentBgcolor}" @click="openColorPicker($event,appearance.contentBgcolor,'contentBgcolor')"><div class="arrow"></div></div>
                        <el-select v-model="appearance.contentBgOpacity" placeholder="请选择" @change="transparencyChange($event, 'contentBgOpacity')">
                          <el-option
                            v-for="item in transparency"
                            :key="item"
                            :label="item"
                ...
                <el-collapse-item name="6">
                  <template slot="title">
                    <i class="el-icon-caret-right"></i>提交区样式
                  </template>
                  <div class="setting-box">
                    <div class="setting-item">
                      <label for="">提交背景</label>
                      <div class="item-content">
                        <div class="color" :style="{background:appearance.commitBgColor}" @click="openColorPicker($event,appearance.commitBgColor,'commitBgColor','commitBgType')"><div class="arrow"></div></div>
                        <el-select v-model="appearance.commitBgOpacity" placeholder="请选择" @change="transparencyChange($event, 'commitBgOpacity')">
                          <el-option
                            v-for="item in transparency"
                            :key="item"
                            :label="item"
                        ...
                        <label for="commitBgImg" class="upload-file">
                          <i class="iconfont icon-icon-test"></i>
                          <input type="file" id="commitBgImg" @change="uploadFile($event, 'commitBgImg', 'commitBgType')">
                        </label>
                      </div>
                    </div>
                    <div class="setting-item">
                      ...
                      <div class="item-content">
                        <div class="color" :style="{background:appearance.commitButtonColor}" @click="openColorPicker($event,appearance.commitButtonColor,'commitButtonColor')"><div class="arrow"></div></div>
                      </div>
                    </div>
                    <div class="setting-item">
                      <label for="">按钮显示</label>
                      <div class="item-content">
                        <el-radio v-model="appearance.commitButtonShow" :label="0" @change="buttonShowChange($event)">单按钮</el-radio>
                        <el-radio v-model="appearance.commitButtonShow" :label="1" @change="buttonShowChange($event)">双按钮</el-radio>
                      </div>
                    </div>
                    <div class="setting-item">
                      <label for="">按钮文字</label>
                      <div class="item-content">
                        <el-input class="button-text"  v-model="commitButtonTextFirst" placeholder="请输入内容" v-if="appearance.commitButtonShow === 1"></el-input>
                        <el-input class="button-text" v-model="commitButtonTextSecond" placeholder="请输入内容"></el-input>
                      </div>
                    </div>
                    <div class="setting-item">
                      <label for="">按钮宽度</label>
                      <div class="item-content">
                        <el-select v-model="commitButtonWidth" value-key="name" placeholder="请选择" @change="buttonWidthChange" style="width: 170px">
                          <el-option
                            v-for="item in buttonWidthList"
                            :key="item.name"
                            :label="item.label"
                            :value="item">
                          </el-option>
                    ...
                    <div class="setting-item">
                      <label for="">按钮位置</label>
                      <div class="item-content">
                        <div class="button-tab">
                          <a :class="{active: appearance.commitButtonPosition === 0}" @click="buttonPositionChange(0)">居左</a>
                          <a :class="{active: appearance.commitButtonPosition === 1}" @click="buttonPositionChange(1)">居中</a>
                          <a :class="{active: appearance.commitButtonPosition === 2}" @click="buttonPositionChange(2)">居右</a>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <a class="setting-button" @click="setModule = true"><i class="iconfont icon-xiayibu-o"></i>模块外观</a>
        </div>
      </el-collapse-transition>
    </div>
    <div class="colum-setting" v-if="settingsType === 1">
      ...
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item name="28">
          ...
          <div class="input-box">
            <el-input v-model="myColumn.title" placeholder="请输入内容..."></el-input>
          </div>
        </el-collapse-item>
        <!-- 分栏-显示 -->
    ...
    <div class="field-setting" v-if="settingsType === 2">
      ...
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item name="1">
          ...
          <div class="input-box">
            <el-input v-model="myComponent.label" placeholder="请输入内容..." maxlength="25"></el-input>
          </div>
        ...
        <el-collapse-item name="2" v-if="isShowChooseType">
          ...
          <div class="input-box">
            <el-select v-model="defaultChooseType" @change="modelChange('chooseType',$event)" placeholder="请选择" :disabled="myComponent.name === 'personnel_principal'">
        ...
        <el-collapse-item name="12" v-if="myComponent.type === 'mutlipicklist'">
          ...
          <div class="add-btn">
            <el-button plain size="mini" icon="el-icon-plus" @click="handleAddItem(myComponent.type)">添加选项</el-button>
          </div>
        ...
        <el-collapse-item name="15" v-if="isFormula">
          ...
          <div class="add-btn">
            <el-button plain size="mini" icon="el-icon-plus" @click="showformulaSetting(myComponent.type, myComponent.name)">设置公式</el-button>
          </div>
          <div class="input-box">
            <label for="">返回类型</label>
            <el-select placeholder="请选择类型" v-model="formulaReturnVal" @change="modelChange('formulaFormat',$event)">
          ...
          <div class="input-box" v-show="myComponent.field.numberType === '4'">
            <label for="">类型</label>
            <el-select v-model="myComponent.field.decimalLen" placeholder="yyyy-MM-dd">
              <el-option v-for="date in dateList" :key="date.name" :label="date" :value="date">
              </el-option>
            </el-select>
          </div>
        ...
        <el-collapse-item name="18" v-if="myComponent.type === 'reference'">
          ...
          <div class="input-box">
            <el-select v-model="referenceModuleModel" value-key="id" @change="modelChange('referenceModule',$event)">
          ...
          <div class="input-box">
            ...
            <el-select v-model="referenceFieldModel" value-key="value" @change="modelChange('referenceField',$event)">
        ...
        <!-- 子表单样式 -->
        <el-collapse-item name="28" v-if="myComponent.type === 'subform'">
          <template slot="title">
            <i class="el-icon-caret-right"></i>子表单样式
          </template>
          <div style="padding-bottom:20px;">
            <el-radio v-model="myComponent.field.formStyle" label="0">表格</el-radio>
            <el-radio v-model="myComponent.field.formStyle" label="1">卡片</el-radio>
          </div>
        </el-collapse-item>
        <!-- 子表单 -->
        ...
        <!-- 字段宽度 -->
        <el-collapse-item name="22" v-if="myComponent.type !== 'subform' && myComponent.type !== 'attachment' && myComponent.type !== 'picture' && myComponent.type !== 'multitext'">
          <template slot="title">
    ...
    <transition name="slide">
      <custom-subform v-if="settingsType === 3" :subformComponent="subformComponent" :components="allField" :subformName="subformName"></custom-subform>
    </transition>
  </el-container>
</template>
<script>
import { mapState, mapGetters, mapMutations } from 'vuex'
import {subformList, themes, barcodeStyleList, formulaReturn1, formulaReturn2} from '@/common/js/constant.js'
import {HTTPServer} from '@/common/js/ajax.js'
import draggable from 'vuedraggable'
import CustomSubform from '@/backend/custom/components/custom-subform'
import InputNumber from '@/common/components/input-number'
import ColorPickerPopover from '@/common/components/color-picker-popover'
import UploadImgSingle from '@/common/components/upload-img-single'
export default {
  name: 'CustomDragRight',
  components: {
    draggable,
    CustomSubform,
    InputNumber,
  ...
  data () {
    return {
      ...
      themeList: themes,
      appearance: {},
      transparency: ['不透明', '90%', '80%', '70%', '60%', '50%', '40%', '30%', '20%', '10%'],
      buttonNumber: '1',
      buttonWidthList: [{name: 0, label: '自适应'}, {name: 1, label: '与表单同宽'}],
      // myApplication: [],
      myModule: {},
      myColumn: {},
      myComponent: {
  ...
  methods: {
    ...
    shiftTheme (theme) {
      ...
      appearance.pageBgColor = theme.color
      appearance.headerBgType = 2
      appearance.headerBgImg = theme.background
      this.setCustomAppearance(appearance)
    },
    // 打开颜色选择器
    openColorPicker (evt, color, name, type) {
      let data = {
        color: color,
        name: name,
        type: type,
        top: evt.clientY - evt.offsetY + 40,
        left: evt.clientX - evt.offsetX
      }
      this.$bus.emit('openColorPickerPopover', data)
    },
    // 上传图片
    uploadFile (event, name, type) {
      ...
      HTTPServer.imageUpload(formdata).then((res) => {
        ...
        if (res[0] && res[0].file_url) {
          let appearance = JSON.parse(JSON.stringify(this.custom_appearance))
          appearance[name] = res[0].file_url
          appearance[type] = 1
          this.setCustomAppearance(appearance)
        }
      })
    ...
    // 透明度变化
    transparencyChange (item, name) {
      let appearance = JSON.parse(JSON.stringify(this.custom_appearance))
      appearance[name] = item
      this.setCustomAppearance(appearance)
    },
    // 提交区按钮显示变化
    buttonShowChange (code) {
      let appearance = JSON.parse(JSON.stringify(this.custom_appearance))
      appearance.commitButtonShow = code
      this.setCustomAppearance(appearance)
    },
    // 按钮宽度变化
    buttonWidthChange (item) {
      let appearance = JSON.parse(JSON.stringify(this.custom_appearance))
      appearance.commitButtonWidth = item.name
      this.setCustomAppearance(appearance)
    },
    // 按钮位置变化
    buttonPositionChange (code) {
      let appearance = JSON.parse(JSON.stringify(this.custom_appearance))
      appearance.commitButtonPosition = code
      this.setCustomAppearance(appearance)
    },
    // 获取二维码样式图片
    codeStyleBg (style) {
  ...
  mounted () {
    ...
    this.$bus.on('setFormula', (value) => {
      if (value.name === this.myComponent.name) {
        ...
        this.myComponent.field.bean = value.bean
  ...
  computed: {
    ...
    headerModuleName: {
      set (value) {
        let appearance = JSON.parse(JSON.stringify(this.custom_appearance))
        appearance.headerModuleName = value
        this.setCustomAppearance(appearance)
      },
      get () {
        return this.custom_appearance.headerModuleName
      }
    },
    // 表头模块描述
    headerModuleDescribe: {
      set (value) {
        let appearance = JSON.parse(JSON.stringify(this.custom_appearance))
        appearance.headerModuleDescribe = value
        this.setCustomAppearance(appearance)
      },
      get () {
        return this.custom_appearance.headerModuleDescribe
      }
    },
    // 提交区按钮第一个文字
    commitButtonTextFirst: {
      set (value) {
        let appearance = JSON.parse(JSON.stringify(this.custom_appearance))
        appearance.commitButtonTextFirst = value
        this.setCustomAppearance(appearance)
      },
      get () {
        return this.custom_appearance.commitButtonTextFirst
      }
    },
    // 提交区按钮第二个文字
    commitButtonTextSecond: {
      set (value) {
        let appearance = JSON.parse(JSON.stringify(this.custom_appearance))
        appearance.commitButtonTextSecond = value
        this.setCustomAppearance(appearance)
      },
      get () {
        return this.custom_appearance.commitButtonTextSecond
      }
    },
    // 提交区按钮宽度
    commitButtonWidth: {
      set (value) {
        let appearance = JSON.parse(JSON.stringify(this.custom_appearance))
        appearance.commitButtonWidth = value.name
        this.setCustomAppearance(appearance)
      },
      get () {
        if (this.custom_appearance.commitButtonWidth === 0) {
          return {name: 0, label: '自适应'}
        } else {
          return {name: 1, label: '与表单同宽'}
        }
      }
    }
  },
  watch: {
    ...
    setModule (newVal) {
      this.$bus.emit('switchAppear', newVal)
      if (!newVal) {
    ...
    custom_layout: {
      handler: function (val, oldval) {
        this.myModule = JSON.parse(JSON.stringify(val))
      },
      deep: true
    },
    custom_appearance: {
      handler: function (val, oldval) {
        if (Object.keys(oldval).length !== 0) {
          this.appearance = JSON.parse(JSON.stringify(val))
        }
      },
      deep: true
    }
