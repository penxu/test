...
<div class="drmain-container matrix-container">
  <v-table id="reportTable" is-horizontal-resize style="width:100%; height: 100%;" :columns="columns" :table-data="tableData" :cell-merge="cellMerge"></v-table>
  <!-- <v-table id="reportTable" style="width:100%; height: 100%;" :columns="columns" :table-data="tableData" :cell-merge="cellMerge"></v-table> -->
</div>
</template>
<script>
export default {
  ...
  mounted () {
    // 聚合表发送的数据
    this.$bus.off('reportDatasUpdate')
    this.$bus.on('reportDatasUpdate', val => {
      console.log(val)
      this.isJuhe = true
  ...
  methods: {
    ...
    cellMerge (rowIndex, rowData, field) {
      ...
      if (this.isJuhe) {
        if (rowIndex % 2 === 0) {
          // '偶数'
          content = '<span style="background-color:#F7F8FA;;color: #333333;">' + (rowData[field] === '未知' ? '-' : rowData[field] || '0') + '</span>'
        } else {
          // '奇数'
          content = '<span style="background-color:#ffffff;color: #333333;">' + (rowData[field] === '未知' ? '-' : rowData[field] || '0') + '</span>'
        }
      }
      var jsondata = {
        colSpan: 1,
...
.v-table-views{
  ...
  .v-table-btable{
    .v-table-row{background-color: #fff;
      td{color: #4a4a4a;}
      .v-table-body-cell{
