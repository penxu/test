  ...
  <div class="custom-drag-center-wrip">
    ...
    <div class="colum-box">
      <draggable v-model="enableLayout" :options="dragOptions" @add="dragAdd($event)" @update="dragUpdate($event)" class="box-item clear">
        <div v-for="(item,index) in enableLayout" class="components" :key="item.name" @click="settings($event,item,index)" :class="{'width-100':item.width === '100%','structure-type':item.field.structure === '0',
        'active-compons':item.active === '1'}">
          <span><i class="red" v-if="item.field.fieldControl === '2'">*</i>{{item.label}}</span>
          <div class="type-text">{{item.typeText}}</div>
          <div class="delete-buttond" v-if="item.remove === '1'" @click.stop="deleteComponent(index)">
...
export default {
  ...
  methods: {
    ...
    dragAdd (evt, line) {
      ...
      // this.setSingeLayout(this.columnNumber)
    ...
    deleteComponent (index) {
      ...
      // this.$bus.$emit('sendTaskUnusedField', 'delete', rmItem)
      let obj = {
        type: 'delete',
        field: rmItem
      }
  ...
  created () {
    if (this.enableLayout.length === 0) {
      this.enableLayout = JSON.parse(JSON.stringify(taskDefaultFields))
      console.log(this.enableLayout, '8888888')
    }
  },
  computed () {
    // ...mapState({
  ...
  mounted () {
    // this.$bus.off('sendAllLayout')
    // this.$bus.on('sendAllLayout', (isExit, value) => {
    //   console.log(isExit, 'isExit中间')
    //   if (!isExit) { // 不存在布局
    //     value.enableLayout = JSON.parse(JSON.stringify(taskDefaultFields))
    //     this.enableLayout = value.enableLayout
    //     console.log(this.enableLayout, 'enablelaytout')
    //   } else { // 存在布局

    //   }
    // })
    // this.$bus.emit('sendTaskProperty', false, this.enableLayout)
  }
}
</script>
