...
import * as employeeJs from '../../common/js/employee.js'
export default {
  components: {},
  props: ['centerUser'],
  data () {
  ...
  watch: {
    roleRadioData (data) {
      console.log(data)
      this.prepareData = data.prepareData ? employeeJs.employee.conversion(data.prepareData) : []
  ...
  methods: {
    confirm () {
      ...
      bus.$emit('selectRoleRadio', this.roleRadioData || undefined)
    ...
    checkInit (arr, data, bool) {
      for (var i = 0; i < arr.length; i++) {
        ...
        if (arr[i].type === data.type && arr[i].id === data.id) {
          arr[i].checked = bool
        } else {
          arr[i].checked = false
        }
    ...
    findUsersByCompany () {
      axios({
        method: 'GET',
        url: url.base + 'role/getRoleGroupList',
        params: {},
        ...
        .then((response) => {
          if (response.data.response.code === 1001) {
            ...
            alert(response.data.response.describe)
          }
        })
