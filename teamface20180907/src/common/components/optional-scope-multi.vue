        ...
        <div class="content">
            <div class="left"  :class="'multi' + tabType">
                ...
                <div class="item" v-for="(item,index) in treeData" :key="index" @click="classifyCheck(item, index)">
                    <i class="back iconfont icon-pc-member-company" v-if="tabType === 0 && item.type === 4"></i>
                    <i class="back iconfont icon-pc-member-organ" v-if="tabType === 0 && item.type === 0"></i>
            ...
            <div class="right">
                <div class="item" v-for="(item, index) in prepareData" :key="index" :class="'type' + item.type">
                    <img v-if='item.picture' v-bind:src="item.picture + '&TOKEN=' + token" />
                    <div v-if='!item.picture' class="simpName backImage">{{item.name | limitTo(2)}}</div>
                    <span class="name" :title="item.name">{{item.name}}
                      <span class="postName" v-if="item.post_name">( {{item.post_name}} )</span>
                    </span>
                    <i class="remove iconfont icon-pc-member-delet" @click="removeItem(item, index)"></i>
                </div>
            </div>
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      navList: [],
      seleteForm: false,
      responseData: this.optionalScopeMulti,
      token: JSON.parse(sessionStorage.requestHeader).TOKEN
    }
  },
  watch: {
    optionalScopeMulti (data) {
  ...
  methods: {
    confirm () {
      this.optionalScopeMulti.prepareData = this.prepareData
      console.log(this.optionalScopeMulti)
      this.$bus.emit('select-optional-scope-multi', this.optionalScopeMulti || undefined)
      this.seleteForm = false
    },
    emptying () {
      this.treeData.map((item1, index1) => {
        delete this.treeData[index1].checked
      })
      this.prepareData = []
    },
    removeItem (data, index) {
      for (var i = 0; i < this.treeData.length; i++) {
        if (this.treeData[i].value === data.value) {
          this.treeData[i].checked = false
          break
        }
      }
      this.prepareData.splice(index, 1)
    },
    funInit () {
      this.treeData.map((item1, index1) => {
    ...
    classifyCheck (data, index) {
      ...
      for (var i = 0; i < this.prepareData.length; i++) {
        if (this.prepareData[i].value === data.value) {
          bool = true
          this.prepareData.splice(i, 1)
          break
        }
      }
      this.treeData[index].checked = !bool
      if (!bool) {
        this.prepareData.push(data)
      }
    },
    /** 导航切换 */
    tabCheck (event, data) {
