...
export default {
  ...
  data () {
    return {
      ...
      token: '',
      department: {},
      peopleList: this.property.field.defaultDepartment
    }
  },
  created () {
  ...
  mounted () {
    if (this.property.field.defaultDepartment.length > 0) {
      this.property.field.defaultDepartment.map((item) => {
        if (item.value === '3-current_main_department') {
          item.id = this.department.id
          item.name = this.department.department_name
          item.picture = this.department.picture
          item.value = '0-' + this.department.id
        }
      })
      this.setValue()
    }
