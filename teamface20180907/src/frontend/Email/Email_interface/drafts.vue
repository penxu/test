...
<template>
  <div class="drafts-main-wrip">
    ...
    <div class="Inbox" v-if="judgeParticulars && judgeDustbinParticulars">
      ...
      <!-- <div class="green"  v-if="ParticularsId.timer_task_time === ''" @click="sendBefore"><i class="iconfont icon-shenpi-chaosong" style="margin-right:10px;"></i>发送</div>
      <div class="delete" v-if="ParticularsId.timer_task_time === ''" @click="seleteForm1 = true">定时发送</div> -->
      <!-- 审批通过才显示 -->
    ...
    <div class="contentList" :class="{'for-records':judgeAndRecords}">
      <div class="List">
        <!-- 列表 -->
        <EmailList v-if="judge" :listContent="listContent" :navId="{'label':false, 'id': 3}" :recordObj='recordObj' :style="{ height: judgeAndRecords ? '94%' : '100%'}"></EmailList>
        <!-- 邮件详情 -->
        <EmailParticulars v-if="judgeParticulars"  :id="ParticularsId.id" :judge="judgeSent"></EmailParticulars>
        <!-- <EmailParticulars  :id="ParticularsId.id" :judge="judgeSent"></EmailParticulars> -->
      </div>
    </div>
    <!-- 删除/定时发送弹框 -->
    <div class="mail-dialog">
      ...
      <el-dialog title='定时发送' :visible.sync='seleteForm1'>
...
export default {
  ...
  mounted () {
    ...
    if (this.EmailId) {
      ...
      if (this.EmailId.judgeType !== 0) {
        if (this.EmailId.judgeType === 2 && this.EmailId.sendState) {
          // 已发送
          this.judgeSent = true
          this.judgeSwitch = true
          this.JudgeSentButton = true
          this.judgeDustbinParticulars = false
        } else if (this.EmailId.judgeType === 1 && !this.EmailId.sendState) {
          // 收件箱
    ...
    this.$bus.on('listenEmailParticularsId', (value) => {
      ...
      if ((Number(value.approval_status) === 10 || Number(value.approval_status) === 4) && value.timer_task_time === '') {
        // 如果是普通草稿/已撤销的邮件就跳写信组件
      ...
      } else if (Number(value.approval_status) === 2 || Number(value.approval_status) === 3 || value.timer_task_time !== '') {
        // 定时邮件/审批通过/审批驳回的邮件就开启邮件详情
        this.judge = false
        this.judgeParticulars = true // 开启邮件详情
      }
    })
  ...
  methods: {
    ...
    // 获取列表数据
    getEmailList () {
      var jsondata = {'pageNum': this.pageNum, 'pageSize': this.pageSize, 'boxId': 3}
    ...
    // 获取邮件详情
    getMailDetail () {
      var jsondata = {'id': this.ParticularsId.id, 'type': this.mailType || 1}
      // 复位数据可避免内容不动态变化
      this.ParticularsId = {}
      HTTPServer.getEmailListParticulars(jsondata, 'Loading').then((res) => {
        this.ParticularsId = res
        this.value1 = new Date(this.ParticularsId.timer_task_time)
      })
    },
    // 手动发送
    sendBefore () {
