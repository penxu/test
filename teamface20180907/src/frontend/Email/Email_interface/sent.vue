  ...
  <div class="sent-main-wrip">
    ...
    <div v-if="judgeAndRecords">
      <div class="Inbox">
        ...
        <div class="delete" @click="selectReply('transmit', 2)">转发</div>
      </div>
      <!-- 来往记录 -->
    ...
    <div class="contentList" :class="{'for-records':judgeAndRecords}">
      <div class="List" :class="{'judgeList':judgeAndRecords}">
        <EmailList v-if="judge" :listContent="listContent" :navId="{'label':false, 'id': 2}" :recordObj='recordObj'></EmailList>
...
export default {
  ...
  mounted () {
    ...
    if (this.EmailId) {
      console.log(this.EmailId.selectReply)
  ...
  methods: {
    ...
    // 获取列表数据
    getEmailList () {
      var jsondata = {'pageNum': this.pageNum, 'pageSize': this.pageSize, 'boxId': 2}
