<template>
  <div class="task-approval-detail">
    <!-- 审批详情弹窗 -->
    <!-- <div class="wb-detail"> -->
      ...
      <div class="wb-detail" v-if="wbDtailForm">
        <div class="el-dialog-div">
          ...
          <div class="footer">
            ...
            <!-- 待我审批-操作按钮 -->
            <div class="appr-btns" v-if="((dataDetail.process_status === 0 || dataDetail.process_status === 1) && (navObject.id === 1) && isUnAppr)">
    ...
    <!-- 抄送弹窗 -->
    <div class="chaoson">
      <el-dialog title="抄送"  id="chaosonRX" :visible.sync="chaosonVisible" append-to-body>
    ...
    <!-- (撤销/删除) 弹窗 -->
    <div class="chexiao">
      <el-dialog :title="statusClick.title" id="chexiaoRX" :visible.sync="chexiaoVisible" append-to-body>
    ...
    <!-- （催办/通过/转交）弹窗 -->
    <div class="cuiban">
      <el-dialog :title="statusClick.title" id="cuibanRXnext" :visible.sync="cuibanVisible" append-to-body>
    ...
    <!-- 审批驳回弹框 -->
    <div class="cuiban">
      <el-dialog title="审批" :visible.sync="rejectVisible" id="bohuirxz" append-to-body>
...
import approvalEdit from '@/frontend/approval/approval-edit.vue'/** 审批编辑组件 */
import approvalGraph from '@/frontend/approval/approval-graph.vue'/** 流程图组件 */
...
export default {
  ...
  methods: {
    ...
    getLayoutData () {
      ...
      // let promise = HTTPServer.getEnableLayout(obj).then((res) => {
    ...
    getProgressData () {
      ...
      let promise = ajaxGetRequest(obj, 'workflow/getProcessWholeFlow')
        .then((res) => {
          ...
          let showOrData = JSON.parse(JSON.stringify(res.data.data))
          let sessionData = []
          let status = false
          showOrData.map((v, k) => {
            if (v.task_status_id === '1') {
              sessionData = v.approval_employee_id.split(',')
            }
          })
          sessionData.map((v1, k1) => {
            if (parseInt(this.userInfo.id) === parseInt(v1)) {
              status = true
            }
          })
          this.navObject.id = status ? 1 : 0
          console.log('先')
        })
        .catch((err) => {
          console.log(err)
        ...
        if (data === '通过' || data === '通过-自由流程') {
          ...
          this.submitPassData()
            .then(() => {
              ...
              this.$bus.emit('closeTaskApprovalModal')
            })
        }
        // 转交-提交数据
        if (data === '转交') {
    ...
    rejectSure () {
      ...
      this.submitRejectData()
        .then(() => {
          // 重载审批详情页
          // Promise.all([this.getLayoutData(), this.getDetailData(), this.getProgressData()])
          //   .then(() => {
          //     console.log('已经刷新好详情数据')
          //     this.isUnAppr = false
          //     this.$message({
...
// 审批详情-操作弹框
#chaosonRX,#cuibanRXnext,#bohuirxz {
  .el-dialog {
    width: 540px;
    .el-dialog__header{
      background: none;
...
<style lang="scss">
  #cuibanRXnext .el-dialog__wrapper{
    .el-dialog{
      .el-dialog__header{
        background: none;
        border-bottom: 1px solid #D9D9D9;
